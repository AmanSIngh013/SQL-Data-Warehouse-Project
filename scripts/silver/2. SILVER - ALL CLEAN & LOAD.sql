...
BEGIN
	...
	BEGIN TRY
		...
		prd_nm,
		ISNULL(prd_cost,0) AS prd_cost, --Converting NULLs to Zero
			CASE 
		...
		SET @start_time = GETDATE() ;
		PRINT'>> Truncating Table : silver.erp_customer ';
		TRUNCATE TABLE silver.erp_customer ;

		PRINT'>> Loading Data Into : silver.erp_customer ';
		...
		SELECT 
			...
			CASE 
				 ...
				 ELSE bdate 
			END AS bdate , -- Setting Fututre Birthdate to NULLs
		...
		PRINT'>> Loading Data Into : silver.erp_location '
		INSERT INTO silver.erp_location (cid,cntry)
			...
			REPLACE(cid,'-','') AS cid,
				...
				ELSE TRIM(cntry) 
			END AS cntry -- Mormalise or Handle missing	or blank country codes
		...
		PRINT'>> Loading Data Into : silver.erp_px_cat '
